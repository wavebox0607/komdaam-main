    ...
    return (
        <>
            <div className='bg-white relative rounded-lg hover:bg-green-200 cardShadow group transition-all duration-300 ease-linear h-full flex flex-col mt-5 ml-1 mr-1'>
                ...
                {
                    item?.quantity === '0.00' ? <div className="w-full h-[145px] md:h-[273px] relative p-2 overflow-hidden">
                        ...
                        {parseInt(item?.discount_amount) !== 0 && <div className="absolute z-[3] top-2 right-2 bg-[#ff576d] px-[7px] py-[2px] w-[95px] h-[22px] rounded-md text-white flex justify-center items-center text-xs font-semibold">
                            {parseInt(item?.quantity) === 0 ? <>{bangla ? `স্টক আউট` : 'Out Of Stock'}</> : <> `${parseInt(item?.discount_amount)}% ছাড়` : `${parseInt(item?.discount_amount)}% OFF`</>}
                        </div>}
                    ...
                    </div> : <div className="w-full h-[145px] md:h-[273px] relative p-2 overflow-hidden">
                        <Link to={'/product/' + item?.slug}>
                            <img className='w-full h-full bg-white' src={productImg + item?.images[0]} alt="" />
                        </Link>
    ...
    return (
        <div className="flex justify-between items-center px-2 ">
            ...
            <div className='text-[13px] selection:'>
                ...
                {/* <Taka tk={item?.discount_price ? item?.discount_price : item?.regular_price} className={`text-[#4c9a2a] font-bold`} size={18} /> */}
            </div>
